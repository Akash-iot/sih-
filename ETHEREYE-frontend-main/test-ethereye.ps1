# ETHEREYE Platform Test Script
# Fixes the cmdlet Invoke-WebRequest conflict issue
...
try {
    ...
    Write-Host "   💡 Solution: Make sure backend is running with:" -ForegroundColor Yellow
    Write-Host "      cd ../backend && python -m uvicorn simple_main:app --reload --host 0.0.0.0 --port 8000" -ForegroundColor Gray
}
...
try {
    ...
    Write-Host "   💡 Solution: Make sure frontend is running with:" -ForegroundColor Yellow
...
# Test Crypto Prices
try {
    $priceResponse = Invoke-RestMethod -Uri "http://localhost:8000/api/v1/live/prices" -Method Get -TimeoutSec 5
    Write-Host "   ✅ Crypto Prices API: Working" -ForegroundColor Green
    foreach ($price in $priceResponse.prices) {
        Write-Host "      $($price.coin_id): $$$($price.price) $($price.currency.ToUpper()) (24h: $([math]::Round($price.change_24h, 2))%)" -ForegroundColor Cyan
    }
} catch {
    Write-Host "   ❌ Crypto Prices API Error: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "      Risk Level: $($spiderResponse.metadata.risk_level)" -ForegroundColor Cyan
} catch {
...
# Final Status
Write-Host "🎯 ETHEREYE Platform Status Summary:" -ForegroundColor Magenta
Write-Host "====================================" -ForegroundColor Magenta

$allGood = $true
try {
    $backendTest = Invoke-RestMethod -Uri "http://localhost:8000/health" -Method Get -TimeoutSec 3
    $frontendTest = Invoke-WebRequest -Uri "http://localhost:3000" -Method Get -TimeoutSec 3 -UseBasicParsing
    
    if ($backendTest.status -eq "healthy" -and $frontendTest.StatusCode -eq 200) {
        Write-Host ""
        ...
        Write-Host "   1. Visit: http://localhost:3000/traces.html" -ForegroundColor Gray
    ...
    Write-Host "   Use start_ethereye_complete.bat to start all services." -ForegroundColor Yellow
}
